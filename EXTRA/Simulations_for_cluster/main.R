...
n_list <- c(100, 500, 1000, 10000, 50000) 
...
out <- foreach(j = 1:6, .verbose = T) %dopar% {
  if(j == 1) {
    ### My conjugate prior with a = 2, b = 1
    DPMM(y = y, R = R, burnin = burnin, a = a, b = b, nu0 = nu0, mu0 = mu0, 
         S0 = S0, kappa0 = kappa0)
  } else if (j == 2) {
    ### My conjugate prior with a = 1, b = .1
    DPMM(y = y, R = R, burnin = burnin, a = 1, b = .1, nu0 = nu0, mu0 = mu0, 
         S0 = S0, kappa0 = kappa0)
  } else if (j == 3){
    ...
    outFixed_K <- DPMM(y = y, R = R, burnin = burnin, alpha = 1, alpha_fixed = T,
                       b=b_gamma, nu0 = nu0, mu0 = mu0, S0 = S0, kappa0 = kappa0)
  }
  
}

names(out) <- c("DY_2_1", "DY_1_.1", "DY_10_1", "EW", "Fixed_K", "Fixed_1")
